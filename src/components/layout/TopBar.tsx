...
export default function TopBar({ role, onRoleChange, onCreateTicket }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 flex items-center gap-4 px-8 h-14 bg-white/90 backdrop-blur-md z-30 border-b border-slate-100">
      {/* Search */}
      <div className="flex items-center bg-surface-container-low px-3 py-1.5 rounded-full flex-1 max-w-md border border-outline-variant/30 gap-2">
        ...
        />
        <span className="text-[11px] bg-surface text-slate-400 font-mono px-1.5 py-0.5 rounded border border-slate-200 shrink-0">⌘K</span>
      </div>
      ...
      <button
        ...
        className="flex items-center gap-1.5 bg-primary text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:opacity-90 transition-opacity"
      >
      ...
      <div className="relative">
        ...
        {showNotif && (
          <div
            className="absolute top-12 right-0 w-80 bg-surface-container-high rounded-xl shadow-xl z-50 border border-slate-200 overflow-hidden"
            onClick={e => e.stopPropagation()}
          >
            <div className="px-4 py-3 border-b border-slate-200">
              <p className="text-xs font-bold uppercase tracking-wider text-on-surface-variant">Notificaciones</p>
            </div>
            {HD_NOTIFICATIONS.map(n => (
              <div key={n.id} className="flex gap-3 px-4 py-3 hover:bg-surface-container transition-colors cursor-pointer">
                <Icon name={n.icon} size={18} className="text-primary mt-0.5 shrink-0" />
                <div>
      ...
      <div className="relative">
        ...
        {showRole && (
          <div
            className="absolute top-12 right-0 w-72 bg-surface-container-high rounded-xl shadow-xl z-50 border border-slate-200 overflow-hidden"
            onClick={e => e.stopPropagation()}
          >
            <div className="px-4 py-3 border-b border-slate-200">
              <p className="text-xs font-bold uppercase tracking-wider text-on-surface-variant">Cambiar de rol (demo)</p>
            </div>
            {HD_ROLES.map(r => (
              <button
                key={r.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-on-surface">{r.user.name}</p>
                  <p className="text-xs text-on-surface-variant">{r.name}</p>
                </div>
                {role.id === r.id && <Icon name="check" size={18} className="text-primary" />}
              </button>
            ))}
          </div>
        )}
