...
export default function AgentsPerformance() {
  ...
  return (
    <div className="space-y-8"> <div> <p className="text-xs font-bold uppercase tracking-wider text-secondary mb-1">Equipo</p> <h1 className="text-[32px] leading-10 font-semibold text-on-surface">Rendimiento de agentes</h1> <p className="text-sm text-on-surface-variant mt-1">Carga, productividad y satisfacción del equipo.</p> </div> <div className="bg-white dark:bg-dark-surface-container rounded-xl border border-slate-100 dark:border-dark-outline-variant shadow-sm overflow-hidden"> <table className="w-full text-sm"> <thead> <tr className="border-b border-slate-100 dark:border-dark-outline-variant bg-surface-container-low">
          ...
          <tbody className="divide-y divide-slate-50 dark:divide-dark-outline-variant/30">
            {isLoading ? (
              ...
              return (
                <tr key={entry.rank} className="hover:bg-surface-container-low transition-colors"> <td className="px-5 py-4 font-bold text-on-surface-variant">#{entry.rank}</td>
                  ...
                  <td className="px-5 py-4 min-w-[140px]"> <p className="text-xs mb-1"><strong>{entry.ratingRatePct.toFixed(1)}%</strong> satisfacción</p>
